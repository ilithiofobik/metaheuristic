...
fn tabu_search(
    ...
    while counter < n * 10 && last_change < n {
            ...
            threads.push(thread::spawn(move || {
                ...
                if *swap_change_clone {
                    for i in (t..n - 1).step_by(num_of_threads) {
                        for j in i + 1..n {
                            let new_change = algold::change_value_swap(&perm, &matrix_clone, i, j);
                    ...
                    for i in (t..n - 1).step_by(num_of_threads) {
                        for j in i + 1..n {
                            let new_change = algold::change_value_invert(
                                &perm,
                                &matrix_clone,
                                i,
                                j,
        ...
        if *swap_change {
            algold::swap(&mut perm, global_best_i, global_best_j);
        } else {
            algold::reverse(&mut perm, global_best_i, global_best_j);
        }
...
fn tabu_search_no_threads(
    ...
    while counter < 10 * n && last_change < n {
        ...
        if swap_change {
            for i in 0..n - 1 {
                for j in i + 1..n {
                    let new_change = algold::change_value_swap(&best_perm, matrix, i, j);
            ...
            for i in 0..n - 1 {
                for j in i + 1..n {
                    let new_change =
                        algold::change_value_invert(&best_perm, matrix, i, j, &old_sums, &new_sums);
                    if new_change > global_best_change
                        && (!(tabu_list.contains(&(i, j))) || new_change > glo_change as i64)
                    {
                        global_best_i = i;
        ...
        if swap_change {
            algold::swap(&mut best_perm, global_best_i, global_best_j);
        } else {
            algold::reverse(&mut best_perm, global_best_i, global_best_j);
        }
